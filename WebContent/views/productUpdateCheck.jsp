...
<body>
<%	ShoppingBean shopUpdate = (ShoppingBean) session.getAttribute("shopget"); 
		Map<Integer,String> categorymap =(Map<Integer,String>)session.getAttribute("categorymap");%>
		<%@ include file="adHeader.jsp"%><br>
<h1>変更画面</h1>
<span>以下の内容で変更します。よろしいですか？</span>
<table><tr><td>
商品名</td><td>${shopUpdate.item_name}</td></tr>
<tr><td>価格</td><td>${shopUpdate.price}</td></tr>
<tr><td>カテゴリー </td><td><%
			for (Map.Entry<Integer, String> category : categorymap.entrySet()) {
				if(shopUpdate.getCategory_id()==category.getKey()){%>
				<%=category.getValue()%><% 
			}}
			%></td></tr>
<tr><td>商品画像</td><td>${shopUpdate.item_img}</td></tr>
<tr><td>在庫数</td><td>${shopUpdate.item_value}</td></tr></table>
<form action="../servlets/ProductUpdateServlet" method="post">
<input type="button" name="back"onclick="location.href='../views/productUpdate.jsp'" value="戻る" class ="button2">
