...
<div class="item">
		<%
		List<ShoppingBean> keywordList = (List<ShoppingBean>) session.getAttribute("keywordList");
		if(keywordList.size()==0){%>
			<%=request.getAttribute("keyword") %>　に該当する商品はありません。
		<%}else{
		for (ShoppingBean list : keywordList) {
		%><ul class="product">
		<li><span>
		商品<%=list.getItem_id()%><br>
